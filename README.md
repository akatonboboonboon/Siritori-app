...
- 非公開を既定にした部屋公開設定、安全な10文字コードの招待URL、公開待機部屋一覧
...
- 公開中かつ待機中の部屋だけを返す一覧と、NFKC・大文字小文字・空白をそろえた部屋名の一意制約
...
部屋公開設定などの追加列は、この起動処理がNeonへ自動マイグレーションします。
新しい環境変数の追加や、Neon Consoleでの手作業によるSQL実行は不要です。
