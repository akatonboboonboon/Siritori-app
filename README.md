...
- 待機中または対人戦中に30分間、参加・準備・設定変更・対局進行がない部屋の自動削除
  （保存して再開する1人用Bot戦は対象外）
...
- 毎ターン変わる音数・文字・語尾などの命令と、直近10語の末尾封印に挑む
  Hard Bot固定の「鬼しりとり」（保存・再開・同設定再戦に対応）
...
- 起動時と60秒ごとに、保存された最終更新から30分以上動きがない待機／対人部屋を
  バージョン競合から保護して削除する定期処理
...
相手はHard Bot 1体、ライフは3、1手30秒です。毎ターンの音数指定に加えて、
