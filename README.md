...
- ログインユーザーが単語・ひらがなの読み・任意の補足を送れる審査制の単語追加リクエスト
...
- 申請を送っただけでは、承認状態にかかわらず対局で使える単語へ自動追加しない境界
...
単語追加リクエスト用の`0006_word_suggestions`を含む追加列・テーブルは、
この起動処理がNeonへ自動マイグレーションします。新しい環境変数の追加や、
Neon Consoleでの手作業によるSQL実行は不要です。
...
送信直後にその単語が対局で使えるようになることはありません。辞書への採用には、
出典・品詞・読み・ライセンスを人が確認した別のデータ更新が必要です。
