...
`0004_score_attack_runs`、複数ラウンド再戦用の`0005_room_current_game`を含む
追加列・テーブルは、この起動処理がNeonへ自動マイグレーションします。
