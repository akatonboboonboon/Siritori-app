...
| 2026-07-27 | OpenAI Codex | 待機部屋設定・手番通知 | 応募者本人の追加要望を受け、Codexが待機中の部屋主による最大人数、制限時間、観戦許可、公開／非公開、空席Bot補充の変更を実装した。最大人数・制限時間・Bot補充を変えた場合だけ全readyを同じ更新で解除し、所有者・状態・revision・現在人数をDB側でも再検査する。対局画面では内部IDを出さず表示名で現在手番を示し、本人の人間手番だけ入力カードを赤枠にする。新しい正解または脱落で本人へ手番が移ったときは、効果音ONの場合だけブラウザ生成の鐘音を鳴らす | AI実装／本人手動確認待ち | 応募者本人が2アカウントとPC・スマートフォンで、部屋主だけの設定変更、準備解除の対象、表示名、代行Bot表記、赤枠、鐘音、効果音OFFを確認する | `agent/waiting-room-settings` |
| 2026-07-27 | OpenAI Codex | 直前単語表示・履歴追従 | 応募者本人の要望を受け、Codexが入力欄付近への直前単語と読みの表示、古い順から新しい順の履歴維持、新規追加時だけ末尾へ動く履歴領域、モーション軽減対応を実装した | AI実装／本人手動確認待ち | 応募者本人がPC・スマートフォンで、初手前・漢字語・Bot返答・脱落語の表示と、入力中に履歴が更新された時の追従を確認する | `agent/history-context` |
| 2026-07-27 | OpenAI Codex | 既存語の申請拒否 | 応募者本人の要望を受け、Codexが単語申請の保存前に通常対局と同じSudachi＋承認語辞書を照合し、すでに使用できる表記を申請せず理由表示する処理と回帰テストを実装した | AI実装／本人手動確認待ち | 応募者本人が通常辞書語、承認済み語、未収録語を申請し、前2件だけ拒否され未収録語だけ審査待ちになることを確認する | `agent/reject-existing-suggestions` |
| 2026-07-28 | OpenAI Codex | ライフ制・敗因表示 | 応募者本人が初期ライフ1〜5と、複数人戦で脱落理由・負けた単語が分かる表示を採用した。Codexが全席の残りライフ、単語を含む敗着記録、ライフ0時だけの脱落、降参の即脱落、待機部屋・ソロ・再戦・Neon保存・旧対局互換、対局中と結果の表示、自動マイグレーションを実装した | 自動テスト533件成功／本人手動確認待ち | 応募者本人が2アカウントとBot戦で、ライフ1/5、末尾「ん」・重複・時間切れ・降参、脱落後観戦、表示名・単語・原因・残数、再接続後の復元を確認する | `agent/lives-elimination-feedback` |
| 2026-07-28 | OpenAI Codex | `ヴ` 系と `ブ` 系の接続互換 | 応募者本人が `ヴ` から返す難しさを発見し、接続時だけ `ヴ/ブ`・`ヴァ/バ`・`ヴィ/ビ`・`ヴェ/ベ`・`ヴォ/ボ` を相互互換にする方針を決めた。Codexが人間入力、全難易度Bot、保存対局の開始文字へ同じ規則を適用し、表記・読み・履歴・重複判定は変更しない | AI実装／本人手動確認待ち | 応募者本人がBot戦で `ニューウェーヴ` の後に `ブラジル`、`サンバ` の後に `ヴァイオリン` がつながり、履歴の読みが元のまま表示されることを確認する | `agent/lives-elimination-feedback` |
| 2026-07-29 | OpenAI Codex | デイリー廃止・鬼しりとり | 応募者本人がデイリーチャレンジを廃止し、毎ターンの音数指定に加えて、禁止かな、指定かな、指定語尾、文字継承、音の種類、同じかなの重複禁止、成功直近10語の末尾封印を採用した。初回試遊で10秒では命令と前の単語を確認する時間が足りないと判断し、30秒へ変更した。CodexがHard Bot 1体・3ライフ・30秒の固定ソロ戦、1〜4手目は追加命令1つ・5手目以降は最大2つ、既知候補3語保証、候補不足時の古い封印緩和、人とBot共通のサーバー判定、保存・再開・再戦、レスポンシブ画面を実装した。旧URLは鬼画面へ移し、既存Neon環境を壊さないため旧デイリーのモデル・表・マイグレーションは互換用に残した | AI実装／自動テスト・本人手動確認待ち | 応募者本人がPC・実スマートフォンで全命令、10手の末尾封印、候補不足時の緩和、Hard Botとの難易度、再読み込み・再開・再戦を確認する | `agent/oni-shiritori` |
| 2026-07-29 | OpenAI Codex | 30分放置部屋の自動削除 | 応募者本人の要望を受け、Codexが待機部屋と進行中PvPを保存された最終更新から判定し、起動時と60秒ごとに30分以上動きがない部屋を削除する処理を実装した。対局進行との競合は状態バージョンで新しい操作を優先し、1人用Bot戦、過去Game、画面の定期取得、非永続リアクションは対象から分離した | AI実装／本人手動確認待ち | 応募者本人が公開環境で放置削除、同名再作成、直前に動いた対局の保持、保存Bot戦の保持を確認する | `agent/oni-timer-room-expiry` |
...
- 廃止前のデイリーチャレンジについて、JST日替わり、1日1回、固定3分、途中再開、日付またぎ、公開ランキング、`0007`往復をテストした履歴を保持
...
- `/tutorial`、`/oni-shiritori`、`/admin/word-suggestions`が未ログイン時に安全にログイン画面へ戻り、旧`/daily-challenge`が鬼画面へ移ることを確認
...
- 承認語、鬼しりとり、初回チュートリアル、結果共有の認証済み本番確認
...
5. 鬼しりとりで全命令、成功直近10語の末尾封印、候補不足時の緩和、再開と再戦を確認する
