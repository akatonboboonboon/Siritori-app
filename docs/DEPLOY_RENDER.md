...
`0007_final_features`は審査監査、承認語、旧デイリーチャレンジ保存、チュートリアル完了状態を追加し、
`0008_life_system`は待機部屋のライフ数設定を追加します。旧デイリーの表とマイグレーションは
既存Neon環境の互換性のため残しますが、現在の画面・起動経路からは使用しません。
鬼しりとりは保存対局スナップショットを使うため、新しいマイグレーションや環境変数は不要です。
...
- `/oni-shiritori`がHard Bot 1体・3ライフ・1手30秒で始まり、旧`/daily-challenge`は同画面へ移動する
- 毎ターンの音数指定と追加命令を人とBotが同じように守り、1〜4手目は1命令、5手目以降は最大2命令になる
- 成功した直近10語だけで末尾封印が進み、候補不足時は古い封印だけが必要最小限に緩和される
- 再読み込みや再接続だけでは同じ手番の命令が変わらない
- 待機部屋と進行中PvPをテスト用に30分以上更新なしへし、起動時または次の60秒周期で
  一覧から消えて同名を再利用できる。直前に対局を進めた部屋と保存中のBot戦は消えない
