...
2. Alembicが`0006_word_suggestions`を含む最新版までupgrade
...
新しい環境変数の追加や、Neon ConsoleでのSQL・列・索引の手動作成は不要です。
...
- 無効・削除済み・開始済みの招待URLが部外者へ同じ利用不可表示を返す
- 非公開部屋が公開一覧へ出ず、招待URLからは参加できる
- 公開の待機部屋だけが一覧へ出て、開始後は一覧から消える
