...
選択テーマも対局スナップショットから読み、辞書が返した表記と確定した読みの組を
サーバー側で照合するため、クライアントがテーマ外の実在語を送っても受理しません。
...
公開ゲームと認証画面の基盤はありますが、ロビー、部屋、対戦・観戦、テーマ／Bot／時間の
設定、タイマー警告の最終NiceGUI画面は応募者本人の実装領域です。Easy Botとテーマ分類
データも本人が追加します。PCと実スマートフォンでのQA、およびWindows sandboxのACL
エラーで実施できなかった実ブラウザの見た目確認は、公開前に本人が行います。
