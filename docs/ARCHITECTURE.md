...
鬼しりとりは別の対局実装を作らず、通常のソロBot戦へ`RoomSnapshot.rule_set=oni`を付けて
同じ手番、ライフ、タイマー、中断・再開、再戦の状態機械を再利用します。スナップショットJSONの
schema version 5でルール種別を保存し、version 2〜4の旧保存対局は`standard`として読み込みます。
SQL列は追加しないため、新しいAlembicマイグレーションは不要です。

`shiritori/oni_rules.py`はNiceGUI、DB、Sudachiから独立した純粋なルール層です。読みの音数、
禁止かな、指定かな、指定語尾、文字継承、音の種類、同一かなの重複禁止、成功直近10語の
末尾封印を1つの制約集合として扱います。アプリケーション層は現在の開始文字、使用済み語、
成功履歴から手番ごとの命令を決定し、同じ制約集合で人間の送信を再検査し、Bot候補も絞ります。
ブラウザの表示や申告は判定の正本にしません。

命令はHard Bot 1体、3ライフ、30秒の固定戦で、1〜4手目は音数指定と追加命令1つ、
5手目以降は最大2つを組み合わせます。固定Bot索引で安全な候補が5語以上残る組み合わせだけを
採用します。末尾封印で5語未満になる場合は、最古の封印から必要最小限だけ外し、その数を
表示層へ渡します。封印窓は受理された成功語だけから再構築するため、失敗入力、末尾`ん`、
重複、時間切れ、presence更新では進みません。同じ保存状態からは同じ命令を再現し、再読み込みや
再接続だけで引き直せないようにします。

デイリーチャレンジの画面と起動時サービスは削除し、旧`/daily-challenge`は
`/oni-shiritori`へリダイレクトします。既存Neon環境のマイグレーション履歴を壊さないため、
旧モデル、サービス実装、`daily_challenge_runs`テーブル、`0007_final_features`は残しますが、
現在のアプリケーション起動経路からは読み書きしません。
...
`WAITING`中は現在の部屋主だけが、既存のRoom列に保存される最大人数、制限時間、
観戦許可、公開設定、空席Bot補充の5項目を変更できます。サービスとSQLリポジトリは
部屋状態、所有者、期待revisionをロック中に再確認し、現在のプレイヤー数より小さい
最大人数や、古い画面からの上書きを拒否します。最大人数、制限時間、Bot補充のいずれかが
変わると全プレイヤーのreadyを同じトランザクションで解除します。観戦許可と公開設定だけの
変更ではreadyを維持し、観戦許可をOFFにしても既存の観戦会員を削除せず、新規参加だけを
停止します。
...
アプリ起動時と起動後60秒ごとに、30分以上保存状態が変わっていない部屋を最大100件ずつ
確認します。待機部屋は`Room.updated_at`を行ロック中に再検査して閉鎖し、会員を退出状態へ
移します。進行中PvPは`Room.current_game_id`と一致する`ACTIVE`のGameだけを候補にし、
`Room.updated_at`と`Game.updated_at`がともに期限以前であることを確認します。

進行中PvPの削除は読み取った`state_version`を使うCASとして実行します。候補抽出後に単語、
タイムアウト、復帰などが確定してバージョンが進んだ場合、古い自動削除は失敗し、新しい状態を
残します。成功時は既存の削除処理でGameを`abandoned`、Roomを`closed/deleted`にし、
RoomHubへ理由`inactive`の終了イベントを送ります。現在のRoomへ結び付かない過去Gameと、
保存再開が必要な`SOLO_BOT`は対象外です。既存の更新日時と索引を使うためDB変更は不要です。
...
Bot候補CSVとBot索引は変更しません。ランキング対象の`SQLAlchemyScoreAttackService`は、
同じルール版の途中で審査結果により語彙が変わらないよう、固定したSudachi validatorを使います。
鬼しりとりの命令生成も固定Bot索引だけで候補数を保証しますが、人間が送る語そのものは
通常の`ApprovedLexiconValidator`と同じ入力辞書で検査してから鬼命令を追加検査します。
...
対局画面は、スナップショット中の席所有者IDをまとめて`AuthService`へ渡し、DBの表示名へ
解決してから現在手番を描画します。本人は「あなた（表示名）」、切断代行中は
「表示名の代行Bot」とし、解決できない場合だけ「プレイヤーN」へフォールバックして
内部IDを表示しません。本人が操作する人間席の手番中だけ入力カードへ
`game-turn-card--mine`を付け、赤枠で常時識別できるようにします。

新しい正解または脱落イベントによって本人の人間手番へ移った場合は、既存の状態遷移効果音を
鐘音へ差し替えます。初回描画、同じスナップショットのポーリング、代行Botの手番では鳴らさず、
効果音OFFも既存の設定どおり尊重します。
...
- 対局中は入力欄の近くに直前の単語を表示し、履歴追加時だけ履歴領域を末尾へ移動する
