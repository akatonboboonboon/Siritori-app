...
Neon Free PostgreSQL
  ├─ users / sessions
  ├─ rooms / members / seats
  └─ games / moves / theme_key（スキーマ・履歴用）
```
...
新規部屋は非公開が既定です。公開一覧のクエリは`is_public=true`かつ`WAITING`の行だけを
作成順で返し、開始済み・削除済み・非公開の部屋を漏らしません。公開設定にかかわらず、
参加コードを含む同一オリジンの`/join/{room_code}` URLから参加できます。

新規の参加コードは紛らわしい文字を除いた32文字種から10文字を生成し、既存の短いコードも
読み込み時は互換維持します。招待参照はアカウントとIPの両方で回数を制限し、部屋の
参加者でないユーザーには無効・削除済み・開始済みを同じ利用不可表示にします。
...
次の生存席へ進めます。ソロ戦は人間の即時敗北です。降参済み状態はpresenceから独立して
保存するため、再接続によって取り消されません。
...
自動適用されます。既存の環境変数をそのまま使い、Neon Consoleで列や索引を手作業で
追加しません。
...
公開ゲーム、認証、ロビー、部屋、対戦・観戦、Bot／時間の設定画面まで実装済みです。
